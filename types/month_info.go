...
func NewMonthInfo(moment time.Time, s Salary, v Vacations, h Holidays, history ReportEntries) MonthInfo {
	return MonthInfo{
		moment:    moment,
		salary:    s,
		vacations: v,
...
func (m MonthInfo) workingDays() []time.Time {
	days := make([]time.Time, 0, m.moment.Day())
...
func (m MonthInfo) needReporting() []time.Time {
	var need []time.Time
